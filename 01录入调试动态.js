...
(async function() {
    ...
    const BASES = {
        'Base1': 'appWNNByUsenTcJML', // Base ID 1
        'Base2': 'appXXXXX', // Base ID 2（请替换为你的第二个 Base ID）
    };
    ...
    form.addEventListener('submit', async (event) => {
        ...
        recordData['记录时间'] = new Date().toISOString(); // 当前时间
    ...
    function updateFields() {
        ...
        fieldsContainer.innerHTML = fields.map(field => {
            return `
                ...
                <input type="${field.type === 'multilineText' ? 'textarea' : 'text'}" id="${field.name}" required />
                <br/><br/>
            `;
        }).join('');
    }
